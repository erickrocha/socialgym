...
**Goal:** Add a `/settings` page in `lapidation_mobile` where the user edits language, notification preference, context-menu position, and home page, persisted via the `workout` service's gRPC `SettingsService`.
...
- Work happens only in `lapidation_mobile/` (its own git repo). The `workout` backend is complete and out of scope — do not edit anything under `workout/`.
...
**Files:**
- Run: `lapidation_mobile/tool/generate_proto.sh` (no edits — script already handles all `proto/*.proto` files)
...
Expected: FAIL with "Target of URI doesn't exist: 'package:lapidation_mobile/services/grpc/grpc_settings_service.dart'" (file doesn't exist yet).
