...
void main() {
  group('GrpcSettingsService.positionToWire', () {
    test(
      'capitalizes the position name so the backend Position::from_string matches it',
      () {
        expect(
          GrpcSettingsService.positionToWire(ContextMenuPosition.left),
          'Left',
        );
        expect(
          GrpcSettingsService.positionToWire(ContextMenuPosition.top),
          'Top',
        );
        expect(
          GrpcSettingsService.positionToWire(ContextMenuPosition.right),
          'Right',
        );
        expect(
          GrpcSettingsService.positionToWire(ContextMenuPosition.bottom),
          'Bottom',
        );
      },
    );
  ...
  group('GrpcSettingsService.toProto / toDomain', () {
    test(
      'round-trips a fully-populated Settings through the proto message',
      () {
    ...
    test(
      'toDomain parses a lowercase contextMenuPosition from the backend without erroring',
      () {
        final proto = $settings.Setting(
