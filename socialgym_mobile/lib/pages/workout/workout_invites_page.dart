...
class _WorkoutInvitesContentState extends State<_WorkoutInvitesContent>
  ...
  void _fetch() {
    ...
    context.read<WorkoutInviteProvider>().fetch(
      ...
      businessProfileId:
          widget.isProfessional ? personProvider.activeBusinessProfile?.id : null,
    );
  }
  ...
  Future<void> _accept(Workout workout) async {
    ...
    _showActionSnackBar(
        success, success ? l10n.workoutInviteAccepted : l10n.workoutInviteActionError);
    if (success && mounted) {
  ...
  Future<void> _reject(Workout workout) async {
    ...
    _showActionSnackBar(
        success, success ? l10n.workoutInviteRejected : l10n.workoutInviteActionError);
  }
  ...
  Future<void> _cancel(Workout workout) async {
    ...
    _showActionSnackBar(
        success, success ? l10n.workoutInviteCancelled : l10n.workoutInviteActionError);
  }
                  ...
                  children: [
                    TabBarView(
                      ...
                      children: [
                        ...
                        if (widget.isProfessional) _buildSentTab(provider, l10n),
                      ],
                    ),
      ...
      child: Row(
        children: [
          Icon(Icons.assignment_turned_in,
              color: AppColors.primaryFor(widget.businessType), size: 28),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(l10n.workoutInvitesTitle,
                    style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                Text(l10n.workoutInvitesDescription,
      ...
      child: TabBar(
        ...
        tabs: [
          ...
          if (widget.isProfessional)
            _buildBadgedTab(l10n.workoutInvitesTabSent, (p) => p.pendingSentCount),
        ],
      ),
    );
  }

  Widget _buildBadgedTab(String label, int Function(WorkoutInviteProvider) countOf) {
  ...
  Widget _buildReceivedTab(WorkoutInviteProvider provider, AppLocalizations l10n) {
