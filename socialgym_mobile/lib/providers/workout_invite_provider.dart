...
class WorkoutInviteProvider extends ChangeNotifier {
  ...
  int get pendingSentCount =>
      _sent.where((w) => w.status == 'Pending').length;
  ...
  Future<void> fetch({required String personUuid, int? businessProfileId}) async {
    ...
    try {
      final owned =
          await GrpcWorkoutService.getWorkoutsByOwnerUuid(ownerUuid: personUuid);
      _received = owned
          .where((w) => w.status == 'Pending' && w.assignedByProfileUuid != null)
          .toList();
  ...
  Future<void> _resolveNames() async {
    ...
    final personUuids = _sent
        .map((w) => w.ownerUuid)
        .toSet()
        .where((uuid) => uuid.isNotEmpty && !_recipientByUuid.containsKey(uuid));
