...
class _AddressFormFieldsState extends State<AddressFormFields> {
  ...
  void initState() {
    super.initState();
    _addressLine1Controller = TextEditingController(text: widget.initialAddressLine1 ?? '');
    _addressLine2Controller = TextEditingController(text: widget.initialAddressLine2 ?? '');
    _localityController = TextEditingController(text: widget.initialLocality ?? '');
    _administrativeAreaController =
        TextEditingController(text: widget.initialAdministrativeArea ?? '');
    _postalCodeController = TextEditingController(text: widget.initialPostalCode ?? '');
    _selectedCountry =
        context.read<ResourceProvider>().getCountryByCode(widget.initialCountryCode);
  ...
  void _handleSave() {
    widget.onSave(AddressFormValues(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                widget.isEditing ? Icons.edit_location_alt : Icons.add_location_alt,
              ...
              child: Column(
                ...
                children: _suggestions.map((candidate) {
                  return ListTile(
                    ...
                    title: Text(candidate.formattedAddress, overflow: TextOverflow.ellipsis),
          ...
          Row(
            ...
            children: [
              TextButton(onPressed: widget.onCancel, child: Text(l10n.buttonCancel)),
              const SizedBox(width: 12),
              ElevatedButton.icon(
