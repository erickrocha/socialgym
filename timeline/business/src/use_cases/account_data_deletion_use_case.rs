...
impl AccountDataDeletionUseCase {
    ...
    pub async fn delete_all_for_person(db: &Database, person_uuid: &str) -> Result<(), BusinessError> {
        log::info!("Deleting all timeline data for person_uuid={}", person_uuid);

        PostGateway::new(db).pull_reactions_by_author(person_uuid).await?;
        PostGateway::new(db).pull_comments_by_author(person_uuid).await?;
        PostGateway::new(db).delete_all_by_author(person_uuid).await?;
        EvolutionCheckInGateway::new(db).delete_all_by_person(person_uuid).await?;
        WorkoutSessionGateway::new(db).delete_all_by_person(person_uuid).await?;
        MentionNotificationGateway::new(db).delete_all_involving_person(person_uuid).await?;
