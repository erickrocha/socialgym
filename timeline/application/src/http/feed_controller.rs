...
use crate::http::json::error_response_json::{ForbiddenErrorJson, InternalServerErrorJson, UnauthorizedErrorJson};
...
pub async fn get_feed(
	...
	match PostUseCase::get_feed(&state.database,current_user.person_id,current_user.person_uuid.clone(),page).await
	{
		Ok(posts) => {
			...
			for key in unique_keys {
				match MediaUseCase::generate_cloud_front_signed_url(&key).await {
					...
					Err(e) => {
						log::warn!("Failed to generate CloudFront URL for object_key '{}': {}",key,e.message);
					}
				}
			}
		...
		Err(e) => {
			log::error!("Error fetching feed: {}", e.message);
			Err(ExceptionResponse::InternalServerError(locale,ErrorKey::FeedFetchFailed))
		}
	}
}
...
pub async fn get_feed_by_uuid(
	...
	Extension(locale): Extension<Locale>) -> HttpResponse<Json<Vec<PostJson>>> {
	let page = params.page.unwrap_or(0);
