...
impl AccountPurgeUseCase {
	...
	pub async fn purge_due_accounts(db: &DbConn) -> Result<u32, BusinessError> {
		let due = UserGateway::find_due_for_deletion(db, Utc::now())
			...
			.map_err(|e| {
				...
				BusinessError::infrastructure("Failed to query accounts due for deletion".to_string())
			})?;
	...
	async fn purge_workout_data(db: &DbConn, user_id: i32, person_id: i32) -> Result<(), BusinessError> {
		...
		let map_err = |e: sea_orm::DbErr| BusinessError::infrastructure(format!("Purge cascade failed: {}", e));

		TokenRevocationGateway::delete_all_by_user_id(&txn, user_id).await.map_err(map_err)?;

		for business_profile in business_profiles {
			BusinessProfileAddressGateway::delete_all_by_business_profile_id(&txn, business_profile.id)
				.await
				.map_err(map_err)?;
			...
			ProfileGateway::delete_all_by_business_profile_id(&txn, business_profile.id)
				.await
				.map_err(map_err)?;
			BusinessProfileGateway::delete_by_id(&txn, business_profile.id).await.map_err(map_err)?;
		}

		WorkoutGateway::delete_all_by_owner_id(&txn, person_id).await.map_err(map_err)?;
		TeamMemberGateway::delete_all_by_person_id(&txn, person_id).await.map_err(map_err)?;
		ProfileGateway::delete_all_by_person_id(&txn, person_id).await.map_err(map_err)?;
		FriendGateway::delete_all_involving_person(&txn, person_id).await.map_err(map_err)?;
		SettingsGateway::delete_by_person_id(&txn, person_id).await.map_err(map_err)?;
		PersonInfoGateway::delete_by_person_id(&txn, person_id).await.map_err(map_err)?;
		PersonAddressGateway::delete_all_by_person_id(&txn, person_id).await.map_err(map_err)?;
		UserGateway::delete_by_person_id(&txn, person_id).await.map_err(map_err)?;
		// Cascades exercise / workout_exercise / person_media automatically.
		PersonGateway::delete_by_id(&txn, person_id).await.map_err(map_err)?;
