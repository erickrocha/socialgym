...
impl GrpcSettingService {
	...
	fn proto_to_domain(setting: Setting) -> Settings {
		Settings {
			id: if setting.id > 0 { Some(setting.id) } else { None },
			uuid: if setting.uuid.is_empty() { None } else { Some(setting.uuid) },
...
impl SettingsService for GrpcSettingService {
	async fn get_by_id(&self, request: Request<SettingIdRequest>) -> Result<Response<Setting>, Status> {
		...
		match result {
			...
			Err(e) => {
				log::error!("Error getting settings by id: {:?}", e);
				Err(Status::not_found("Settings not found"))
			}
		}
	}

	async fn persist_settings(&self, request: Request<Setting>) -> Result<Response<Setting>, Status> {
		...
		match result {
			...
			Err(e) => {
				...
				Err(Status::internal("Error persisting settings"))
			}
		}
	}
	...
	async fn get_by_uuid(&self, request: Request<SettingIdRequest>) -> Result<Response<Setting>, Status> {
		...
		match result {
			...
			Err(e) => {
				log::error!("Error getting settings by uuid: {:?}", e);
				Err(Status::not_found("Settings not found"))
			}
		}
	}
	...
	async fn get_by_owner_ids(&self, request: Request<SettingOwnerIdRequest>) -> Result<Response<Setting>, Status> {
		...
		match result {
			...
			Err(e) => {
				log::error!("Error getting settings by owner ids: {:?}", e);
				Err(Status::not_found("Settings not found"))
			}
		}
	}
