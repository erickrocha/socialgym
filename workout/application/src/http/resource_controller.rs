...
#[utoipa::path(
	...
	responses(
		(status = 200, description = "Static resources such as countries and provinces", body = ResourceJson),
...
pub async fn get_resource(State(state): State<AppState>,Extension(current_user): Extension<User>, Extension(locale): Extension<Locale>) -> HttpResponse<Json<ResourceJson>> {
	...
	if countries_response.is_err() {
		log::error!("Error loading resource countries : {}", countries_response.err().unwrap());
	...
	if settings_response.is_err() {
		log::error!("Error loading resource settings : {}", settings_response.err().unwrap());
		return Err(ExceptionResponse::NotFound(locale,ErrorKey::ResourcesNotFound));
	}
	...
	Ok(ResourceJson::builder().countries(countries).settings(settings).build().into())
